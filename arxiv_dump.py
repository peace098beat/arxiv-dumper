...
Original file is located at
    https://colab.research.google.com/drive/1rcK2yDUbYbKa5ZgOT8mawUe65-XsYSGU
"""
...
class Search(object):
    ...
    def __repr__(self) -> str:
        return (
            '{}(query={}, id_list={}, start={}, max_results={}, sort_by={}, '
            'sort_order={})'
        ).format(
            _classname(self),
            repr(self.query),
...
def clear_db(csv_file="db.csv.gz"):
  if Path(csv_file).exists():
    os.remove(csv_file)

def append_to_db(df, csv_file="db.csv.gz"):
  if Path(csv_file).exists():
...
def read_db(csv_file="db.csv.gz"):
...
Path(out_dir).mkdir(exists=True, parents=True)
...
cnt = START
for result in big_slow_client.get(searcher):
  res_d = result_to_dict(result)
  buffer.append(res_d)

  if (cnt % 1000) == 0:
    df = pd.DataFrame.from_dict(buffer)
    csv_file = f"{out_dir}/db.{cnt:07d}.csv.gz"
    append_to_db(df, csv_file)
    print("save", csv_file)
